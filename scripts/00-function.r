get_beta <- function(model = c('pcr', 'pls', 'cppls', 'mvr', 'xenv', 'bayes', 'linear', 'cpls',
...
getPredErr <- function(coef, minerr, trueBeta, sigma, use.formula = TRUE, test = if(!use.formula) NULL){
  out <- map_dbl(0:dim(coef)[3], function(cmp){
  ...
  tibble(comp = 0:(length(out) - 1), pred_err = out)
}
shared_legend <- function(..., ncol = length(list(...)), nrow = 1, position = c("bottom", "right")) {
  require(grid)
  require(gridExtra)
  plots <- list(...)
