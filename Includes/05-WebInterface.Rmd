...
All `simrel` parameters can be entered using a simple user interface where vector elements are separated with comma(,) and list elements are separated with  semicolon(;). For instance, the relevant position discussed in the [implementation](#implementation) (section \@ref(implementation)) of this paper can be entered as `1, 6; 2, 5; 3, 4` which is equivalent to R syntax `list(c(1, 6), c(2, 5), c(3, 4))`. An R expression equivalent to the input parameters as shown in Figure - 5(b) can be written as,
