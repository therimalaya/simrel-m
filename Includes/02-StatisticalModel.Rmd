...
\mathbf{y} = \boldsymbol{\mu}_y + \boldsymbol{\beta}^t (\mathbf{x} - \boldsymbol{\mu}_x) + \boldsymbol{\varepsilon}
  (\#eq:linear-model)
\end{equation}
where, $\boldsymbol{\beta}^t$ is a matrix of regression coefficient and $\boldsymbol{\varepsilon}$ is an error term such that $\boldsymbol{\varepsilon} \sim N\left(0, \boldsymbol{\Sigma}_{y|x}\right)$. The properties of the linear model \@ref(eq:linear-model) can be expressed in terms of covariance matrices in \@ref(eq:rand-reg-model).
...
Coefficient of Determination
  : The coefficient of determination matrix \@ref(eq:latent-model) is,
  $$
  \begin{aligned}
...
: Here we assume that $\mathbf{w}$'s are independent and mutinormal distributed with variances 1, hence $\boldsymbol{\Sigma}_{ww} = \mathbf{I}_m$.
...
In the previous section, we discussed about the construction of a covariance matrix for the latent structure. Figure~\@ref(fig:cov-plot-print) (left) shows a similar structure resembling the example here. The three colors represent the relevance with the three latent response components $(w_1, w_2$ and $w_3)$. Here we can see that $`r rel.z[[1]][1]`$ and $`r rel.z[[1]][2]`$ (first and second predictor components of $\mathbf{x}$) have non-zero covariance with $w_1$ (first latent component of response $\mathbf{y}$). In the similar manner other non-zero covariances are self-explanatory. 

```{r cov-plot-print, echo = FALSE, warning = FALSE, message = FALSE, out.width = '33%', fig.asp = 1.2, fig.subcap=c("Relevant Position", "Rotation Matrix", "Relevant Predictors"), fig.cap = 'Simulation of predictor and response variables after orthogonal transformation of predictor and response components by rotation matrices $Q$ and $R$ shown as the upper left and the lower right block matrices in (b).', fig.width=1.7, dpi = 800, fig.pos = "!htb"}
...
## Save TIFF
savePlot(plt1, filename = "images/tiff/cov-plot-print-1.tiff")
savePlot(plt2, filename = "images/tiff/cov-plot-print-2.tiff")
savePlot(plt3, filename = "images/tiff/cov-plot-print-3.tiff")

## Save png
savePlot(plt1, filename = "images/png/cov-plot-print-1.png", dev = "png")
savePlot(plt2, filename = "images/png/cov-plot-print-2.png", dev = "png")
savePlot(plt3, filename = "images/png/cov-plot-print-3.png", dev = "png")

## Print png
knitr::include_graphics(c(
  'images/png/cov-plot-print-1.png', 
  'images/png/cov-plot-print-2.png', 
  'images/png/cov-plot-print-3.png'
))
```

In order to simulate predictor variables $(\mathbf{x})$,  we construct matrix $\mathbf{R}$ which then is used for orthogonal rotation of the predictor components $\mathbf{z}$. This defines a new basis for the same space as is spanned by the predictor components. In principle, there are many possible options for defining a rotation matrix. Among them, the eigenvector matrix of $\boldsymbol{\Sigma}_{xx}$ can be a candidate. However, in this reverse engineering approach both rotation matrices $\mathbf{R}$ and $\mathbf{Q}$ along with the covariance matrices $\boldsymbol{\Sigma}_{xx}$ are unknown. So, we are free to choose any $\mathbf{R}$ that satisfied the properties of a real valued rotation matrix, i.e $\mathbf{R}^{-1} = \mathbf{R}^t$ so that $\mathbf{R}$ is orthonormal and its determinant becomes $\pm 1$. Here the rotation matrix $\mathbf{R}$ should be block diagonal as in figure~\@ref(fig:cov-plot-print) (middle) in order to rotate spaces $\mathcal{S}_1, \mathcal{S}_2 \ldots$ separately. Figure~\@ref(fig:simulated-data) (left) shows the simulated predictor components $\mathbf{z}$ that we are following in our example where we can see that the components $`r rel.z[[1]][1]`$ and $`r rel.z[[1]][2]`$ (relevant for $w_1$) is getting rotated together with an irrelevant component $`r rel.z[[1]][3]`$. The resultant predictors (Figure \@ref(fig:simulated-data), right) $`r rel.x[[1]][-3]`$ and $`r rel.x[[1]][3]`$ will hence also be relevant for $w_1$. In the figure, we can see that components $`r irrelpos.z[-4]`$ and $`r irrelpos.z[4]`$ are not relevant for any responses before rotation, however, the $`r setdiff(irrelpos.x, irrel.x)`$ predictors become relevant after rotation keeping $`r irrel.x[1]`$ and $`r irrel.x[2]`$ still irrelevant.

```{r simulated-data, echo = FALSE, warning = FALSE, message = FALSE, out.width = '45%', fig.cap = 'Simulated Data before', fig.align='center', fig.subcap=c("Before Rotation", "After Rotation"), fig.pos='!htb'}
obsPlt1 <- get_obsplot(sim.obj, base.size = 8,  point.size = 0.5) + plusTheme
obsPlt2 <- get_obsplot(sim.obj, FALSE, base.size = 8,  point.size = 0.5) + plusTheme

## Save TIFF
savePlot(obsPlt1, filename = "images/tiff/simulated-data-1.tiff", width = 7)
savePlot(obsPlt2, filename = "images/tiff/simulated-data-2.tiff", width = 7)

## Save png
savePlot(obsPlt1, filename = "images/png/simulated-data-1.png", width = 7, dev = "png")
savePlot(obsPlt2, filename = "images/png/simulated-data-2.png", width = 7, dev = "png")

## Print Plot
knitr::include_graphics(c(
  "images/png/simulated-data-1.png", 
  "images/png/simulated-data-2.png"
))
```
