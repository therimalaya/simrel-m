...
: In similar manner, a parametric representation of eigenvalues corresponding to $\Sigma_{ww}$ is adopted as,
  ...
  (\#eq:eta-parameter)
  \end{equation}
  Here, the decline of eigenvalues becomes steeper as $\eta$ gets far from zero. At $\eta = 0$, all $w$ will have equal variance. Hence we can write $\Sigma_{ww} = \text{diag}(\kappa_1, \ldots, \kappa_m)$.
...
2. Define, $$\sigma_{ij} = \text{Sign}(\mathcal{S}_i)\sqrt{\frac{\rho_{wj}^2\left|\mathcal{S}_i\right|}{\sum_{k\in\mathcal{P}_j}\left|\mathcal{S}_k\right|}\lambda_i\kappa_j}$$
