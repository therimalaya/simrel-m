...
Above analysis has answered some questions such as how methods works when there exist a true reduced dimension in response space but also arised question like why they perform differently. For example, what is the reason for the decreasing performance of the simultaneous envelope method as the $\rho^2$ values are reduced? Does this depend on the dimensions and spheric shape of the $\mathbf{y}$ envelopes? Since, the example is intended as a demonstration of how `simrel-m` can be used in scientific study, and a more elaborative study would be necessary in order to answer such questions, but for this purpose `simrel-m` would be a powerful tool.
