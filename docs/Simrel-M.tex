...
non-zero regression coefficients \((\beta_j \ne 0)\). If
\(\boldsymbol{e}_j, \; j = 1, 2, \ldots, p\) be the eigenvectors
corresponding to \(\mathbf{X}\), then, for some value of
\(\mathcal{H}\),

\begin{equation}
\mathbf{B} = \mathcal{H} \mathbf{E} 
