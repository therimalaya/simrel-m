...
% \signature{$for(authors)$$for(authors.author)$$authors.author$$sep$\\$endfor$$sep$\\\vspace{$signature-space$}$endfor$}
...
\closing{$closing$}
$endif$
$for(authors)$ \parbox[b]{0.3\textwidth}{$for(authors.author)$$authors.author$$sep$\\$endfor$}$sep$\hfill$endfor$
$if(encl)$
